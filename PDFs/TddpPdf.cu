...
EXEC_TARGET fptype device_Tddp (fptype* evt, unsigned int *funcIdx, unsigned int* indices) {
  ...
  idx[ 9] = indices[3 + idx[0]];
  idx[10] = indices[4 + idx[0]];
  idx[11] = indices[5 + idx[0]];
  idx[12] = indices[6 + idx[0]];
  idx[13] = indices[7 + idx[0]];
  ...
  *funcIdx += 1;
  ...
  int evtNum = (int) FLOOR(0.5 + evt[idx[12]]); 
  ...
  unsigned enr = evtNum*numResonances;
  ...
  for (int i = 0; i < numResonances; ++i) {
    ...
    devcomplex<fptype> matrixelement(thrust::get<0>(wh),
                                     thrust::get<1>(wh));
    matrixelement.multiply(amp_real, amp_imag); 
    sumWavesA += matrixelement; 

    matrixelement = devcomplex<fptype>(thrust::get<2>(wh),
				       thrust::get<3>(wh)); 
    matrixelement.multiply(amp_real, amp_imag); 
