...
EXEC_TARGET fptype device_Landau (const fptype* __restrict evt, fptype* p, unsigned int* paramIdx)
{
  ...
  fptype sigma = cudaArray[*paramIdx + 2];

  //int numObserv = cudaArray[*paramIdx + numParams + 1];
  //int numConsts = cudaArray[*paramIdx + numParams + 1 + numObserv + 1];
  //int numNormal = cudaArray[*paramIdx + numParams + 1 + numObserv + 1 + numConsts];

  //I think we want to have an eventIdx as well?
  //No reason to pass 'p', this is cudaArray 
  fptype x     = evt[0]; 

  //This value is numParams + 1 + numObserv + 1 + numConsts + 1 + numNormal + 1
  //*paramIdx += numParams + 1 + numObserv + 1 + numConsts + 1 + numNormal + 1;
