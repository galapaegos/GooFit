...
EXEC_TARGET fptype device_DalitzVeto (fptype* evt, fptype* p, unsigned int* indices) {
  ...
  idx[6] = indices[2 + idx[0] + 0];
  idx[7] = indices[2 + idx[0] + 1];
  ...
  for (int i = 0; i < numVetos; ++i) {
    ...
    int tmp[3];
    tmp[0] = indices[6 + i3 + 0];
    tmp[1] = indices[6 + i3 + 1];
    tmp[2] = indices[6 + i3 + 2];

    unsigned int varIndex =   tmp[0];
    fptype minimum        = p[tmp[1]];
    fptype maximum        = p[tmp[2]];
