...
fptype* dev_param_array_s1 = 0;
fptype* dev_param_array_s2 = 0;
//fptype* binCenters = 0;
...
__host__ bool PdfBase::parametersChanged () const {
  ...
  for (parIter v = params.begin(); v != params.end(); ++v) {
    if (cachedParams[counter] != host_params[parametersIdx + counter]) return true;
    counter ++;
  }
...
__host__ void PdfBase::storeParameters () const {
  ...
  int counter = 0; 
  for (parIter v = params.begin(); v != params.end(); ++v) {
    cachedParams[counter] = host_params[parametersIdx + counter];
    counter ++;
  }
}
